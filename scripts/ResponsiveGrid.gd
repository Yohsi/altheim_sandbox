...
func _notification(p_what):
	match p_what:
		NOTIFICATION_SORT_CHILDREN:
			var col_minw := {} # Max of min_width  of all controls in each col (indexed by col).
			var row_minh := {} # Max of min_height of all controls in each row (indexed by row).
			var col_expanded := [] # Columns which have the SIZE_EXPAND flag set.
			var row_expanded := [] # Rows which have the SIZE_EXPAND flag set.
...
#			Finally, fit the nodes.
			var col_expand = remaining_space.x / col_expanded.size() if col_expanded.size() > 0 else 0.0
			var row_expand = remaining_space.y / row_expanded.size() if row_expanded.size() > 0 else 0.0
